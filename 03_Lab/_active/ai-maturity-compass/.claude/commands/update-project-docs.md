...
Find the **project root** (the immediate subdirectory of the workspace that contains the active codebase — e.g. `projects/my-project`). This is usually the current working directory or its closest ancestor with a `context/` folder.
...
Show the primary user-facing workflow as a sequence or flowchart. For example: intake → processing → output (replace with your project workflow).
