  ...
  if [ -n "$blocks_output" ]; then
    ...
    if [ -n "$active_block" ]; then
      ...
      if [ -n "$reset_time_str" ] && [ -n "$start_time_str" ]; then
        end_sec=$(to_epoch "$reset_time_str"); now_sec=$(date +%s)
        remaining=$(( end_sec - now_sec )); (( remaining<0 )) && remaining=0
        rh=$(( remaining / 3600 )); rm=$(( (remaining % 3600) / 60 ))
        end_hm=$(fmt_time_hm "$end_sec")

        # Token usage % for the 5-hour window (set CLAUDE_TOKEN_LIMIT env var to override)
        total_block_tokens=$(echo "$active_block" | jq -r '.totalTokens // 0' 2>/dev/null)
        TOKEN_LIMIT="${CLAUDE_TOKEN_LIMIT:-339000000}"
        if [ -n "$total_block_tokens" ] && [ "$total_block_tokens" -gt 0 ] 2>/dev/null; then
          session_pct=$(( total_block_tokens * 100 / TOKEN_LIMIT ))
          (( session_pct > 100 )) && session_pct=100
          session_txt="$(printf '%dh %dm until reset at %s (%d%% tokens)' "$rh" "$rm" "$end_hm" "$session_pct")"
        else
          session_pct=0
