...
def get_compact_manifest():
    """If aggressive compaction ran last session, warn what was stripped."""
    manifest_file = Path(".claude/state/compact-manifest.json")
    if not manifest_file.exists():
        return None
    try:
        m = json.loads(manifest_file.read_text(encoding="utf-8"))
        if m.get("mode") != "aggressive":
            return None
        compressed = m.get("compressed_topics", [])
        active = m.get("active_topics", [])
        at = m.get("compacted_at", "")[:16].replace("T", " ")
        if not compressed:
            return None
        return (
            f"[COMPACT NOTE {at}] Aggressive compaction ran. "
            f"Kept: {', '.join(active) or 'general'}. "
            f"Stripped: {', '.join(compressed)}. "
            f"Run /prime if you need that context back."
        )
    except Exception:
        return None
...
compact_note = get_compact_manifest()
