...
v2 addition:
- Sweeps .claude/worktrees/*/ and commits pending changes in each active worktree
  to its own branch, preventing orphaned uncommitted work when agent sessions end.
"""
...
def get_changed_files_in(cwd):
    """Return changed files for a specific directory (worktree support)."""
    result = run(["git", "status", "--short"], cwd=cwd)
    lines = result.stdout.strip().splitlines()
    files = []
    for line in lines:
        if line.strip():
...
def commit_worktrees():
    """Sweep .claude/worktrees/* and commit any pending changes to their branches."""
    worktrees_root = Path(".claude/worktrees")
    ...
    for wt_dir in sorted(worktrees_root.iterdir()):
        ...
        check = run(["git", "rev-parse", "--is-inside-work-tree"], cwd=str(wt_dir))
        if check.returncode != 0:
            continue

        changed = get_changed_files_in(str(wt_dir))
        if not changed:
            continue

        message = generate_message(changed)
        run(["git", "add", "-A"], cwd=str(wt_dir))
        result = run(["git", "commit", "-m", f"worktree: {message}"], cwd=str(wt_dir))

        if result.returncode == 0:
            branch = run(["git", "branch", "--show-current"], cwd=str(wt_dir)).stdout.strip()
            print(f"Auto-commit (worktree/{wt_dir.name} → {branch}) ✓  {message}")
        else:
            err = result.stderr.strip()
            if "nothing to commit" not in err:
                print(f"Auto-commit worktree/{wt_dir.name} failed: {err}", file=sys.stderr)
...
def main():
    ...
    # Commit pending changes in any active worktrees first
    commit_worktrees()

    # Then commit the main checkout as usual
