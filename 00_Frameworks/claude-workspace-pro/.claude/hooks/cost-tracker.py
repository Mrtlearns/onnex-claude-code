...
NOTE: All paths are resolved relative to __file__ (not CWD) so the hook works
correctly even when Claude has cd'd into a subdirectory during a session.
"""
...
# cost-tracker.py  → .claude/hooks/  → .claude/  → <project-root>
