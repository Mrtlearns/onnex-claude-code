...
Write down every assumption, even obvious ones. Examples:
- "We need a REST API" (why not GraphQL, RPC, or direct DB access?)
- "We need a separate frontend" (why not server-side rendering?)
- "We need authentication" (is it possible not all routes need it?)
- "We need to store this in Postgres" (why not Redis, a file, or in-memory?)
